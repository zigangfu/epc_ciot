...
inline bool gtpu_supported_flags_check(gtpu_header_t* header, srslte::log_ref gtpu_log)
{
  ...
  if ((header->flags & GTPU_FLAGS_VERSION_MASK) != GTPU_FLAGS_VERSION_V1) {
    gtpu_log->console("gtpu_header - Unhandled GTP-U Version. Flags: 0x%x\n", header->flags);
    return false;
  }
  if (!(header->flags & GTPU_FLAGS_GTP_PROTOCOL)) {
    gtpu_log->console("gtpu_header - Unhandled Protocol Type. Flags: 0x%x\n\n", header->flags);
    return false;
  }
  if (header->flags & GTPU_FLAGS_EXTENDED_HDR) {
    gtpu_log->console("gtpu_header - Unhandled Header Extensions. Flags: 0x%x\n\n", header->flags);
    return false;
  }
  if (header->flags & GTPU_FLAGS_PACKET_NUM) {
    gtpu_log->console("gtpu_header - Unhandled Packet Number. Flags: 0x%x\n\n", header->flags);
    return false;
  }
  return true;
}
...
inline bool gtpu_supported_msg_type_check(gtpu_header_t* header, srslte::log_ref gtpu_log)
{
  ...
  if (header->message_type != GTPU_MSG_DATA_PDU && header->message_type != GTPU_MSG_ECHO_REQUEST &&
    ...
    gtpu_log->console("gtpu_header - Unhandled message type: 0x%x\n", header->message_type);
    return false;
  }
  return true;
}
