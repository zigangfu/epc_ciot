...
bool gtpu_write_header(gtpu_header_t* header, srslte::byte_buffer_t* pdu, srslte::log_ref gtpu_log)
{
  ...
  if (!gtpu_supported_flags_check(header, gtpu_log)) {
    gtpu_log->console("gtpu_write_header - Unhandled GTP-U Flags. Flags: 0x%x\n", header->flags);
    return false;
  }

  // msg type
  if (!gtpu_supported_msg_type_check(header, gtpu_log)) {
    gtpu_log->console("gtpu_write_header - Unhandled GTP-U Message Type. Message Type: 0x%x\n", header->message_type);
    return false;
  }
  ...
  if (header->flags & (GTPU_FLAGS_EXTENDED_HDR | GTPU_FLAGS_SEQUENCE | GTPU_FLAGS_PACKET_NUM)) {
    if (pdu->get_headroom() < GTPU_EXTENDED_HEADER_LEN) {
      gtpu_log->console("gtpu_write_header - No room in PDU for header\n");
      return false;
    }
...
bool gtpu_read_header(srslte::byte_buffer_t* pdu, gtpu_header_t* header, srslte::log_ref gtpu_log)
{
  ...
  // flags
  if (!gtpu_supported_flags_check(header, gtpu_log)) {
    gtpu_log->console("gtpu_read_header - Unhandled GTP-U Flags. Flags: 0x%x\n", header->flags);
    return false;
  }

  // message_type
  if (!gtpu_supported_msg_type_check(header, gtpu_log)) {
    gtpu_log->console("gtpu_read_header - Unhandled GTP-U Message Type. Flags: 0x%x\n", header->message_type);
    return false;
  }
