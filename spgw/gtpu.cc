...
spgw::gtpu::gtpu() : m_sgi_up(false), m_s1u_up(false), m_s11u_up(false)
{
...
int spgw::gtpu::init(spgw_args_t* args, spgw* spgw, gtpc_interface_gtpu* gtpc, srslte::log_ref gtpu_log)
{
  ...
  err = init_s11u(args);
  if (err != SRSLTE_SUCCESS) {
    m_gtpu_log->console("Could not initialize the S11-U interface.\n");
    return err;
  }
...
int spgw::gtpu::init_s11u(spgw_args_t* args)
{
  socklen_t sock_len;
  ...
  // Logs
  m_gtpu_log->info("Initializing SPGW S11-U interface.\n");

  // Open Socket
  m_s11u = socket(AF_UNIX, SOCK_DGRAM, 0);
  if (m_s11u < 0) {
    m_gtpu_log->error("Error opening UNIX socket. Error %s\n", strerror(errno));
    return SRSLTE_ERROR_CANT_START;
  }
  m_s11u_up = true;
  ...
  if (bind(m_s11u, (const struct sockaddr*)&m_spgw_addr, sizeof(m_spgw_addr)) == -1) {
    m_gtpu_log->error("Error binding UNIX socket. Error %s\n", strerror(errno));
    return SRSLTE_ERROR_CANT_START;
  }
  return SRSLTE_SUCCESS;
}
...
void spgw::gtpu::send_s11u_pdu(uint32_t s11u_teid, srslte::byte_buffer_t* msg)
{
  // Setup GTP-U header
  srslte::gtpu_header_t header;
  header.flags        = GTPU_FLAGS_VERSION_V1 | GTPU_FLAGS_GTP_PROTOCOL;
 ...
 * Tunnel managment
 */
bool spgw::gtpu::modify_gtpu_tunnel(in_addr_t ue_ipv4, srslte::gtpc_f_teid_ie dw_user_fteid, uint32_t up_ctrl_teid, bool s11u)
{
