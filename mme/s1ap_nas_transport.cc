...
bool s1ap_nas_transport::handle_initial_ue_message(const asn1::s1ap::init_ue_msg_s& init_ue, // emm 01
...
//      err = nas::handle_attach_request(enb_ue_s1ap_id, enb_sri, nas_msg, m_nas_init, m_nas_if, m_s1ap->m_nas_log);
      err = nas::handle_nb_attach_request(enb_ue_s1ap_id, enb_sri, nas_msg, m_nas_init, m_nas_if, m_s1ap->m_nas_log);
      break;
    case LIBLTE_MME_SECURITY_HDR_TYPE_SERVICE_REQUEST:
      ...
      break;
    case 0x4d: // Handle NB-IoT Control Plane Service Request
      m_s1ap_log->console("Received Initial UE message -- Control Plane Service Request\n"); // emm 01
      m_s1ap_log->info("Received Initial UE message -- Control Plane Service Request\n");
      err = nas::handle_nb_control_plane_service_request(
          m_tmsi, enb_ue_s1ap_id, enb_sri, nas_msg, m_nas_init, m_nas_if, m_s1ap->m_nas_log);
      break;
                                                     ...
                                                     struct sctp_sndrcvinfo*               enb_sri)
{
  ...
  // Decrypt message if indicated TODO: tmp hack, recover later
  ...
  switch (msg_type) {
    ...
    case LIBLTE_MME_MSG_TYPE_SECURITY_MODE_COMPLETE:
      ...
      //TODO: Hack the checking for testing. Remove it in the future.
      sec_hdr_type = LIBLTE_MME_SECURITY_HDR_TYPE_INTEGRITY_AND_CIPHERED_WITH_NEW_EPS_SECURITY_CONTEXT;
    ...
    case LIBLTE_MME_MSG_TYPE_TRACKING_AREA_UPDATE_REQUEST:
      ...
      break;
    case 0xeb:
      m_s1ap_log->info("UL NAS: ESM Data Transport\n");
      m_s1ap_log->console("UL NAS: ESM Data Transport\n");
      nas_ctx->handle_esm_data_transport(nas_msg);
      break;
    default:
      ...
      m_s1ap_log->console("Unhandled NAS integrity protected message %x\n", msg_type);
      m_pool->deallocate(nas_msg);
      return false;
  }
