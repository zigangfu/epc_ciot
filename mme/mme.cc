              ...
              srslte::log_ref     s11u_log)
{
  ...
  m_s11u_ep = new s11u_ep();
  if (!m_mme_gtpc->init(m_s11u_ep, m_mme_gtpc_log)) {
    m_s1ap_log->console("Error initializing GTP-C\n");
    exit(-1);
  }

  if (m_s11u_ep->init(s11u_log) != SRSLTE_SUCCESS) {
    m_mme_gtpc_log->console("Error initializing S11-U\n");
    exit(-1);
  }
...
void mme::run_thread()
{
  ...
  int s11u  = m_s11u_ep->get_s11u();

  while (m_running) {
    pdu->clear();
    // TODO: Is this right?
    int max_fd = std::max(s1mme, s11);
    max_fd = std::max(s11u, s11);
