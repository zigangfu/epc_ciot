...
public:
  ...
  bool init(s11u_ep *ep, srslte::log_filter* mme_gtpc_log);
  bool send_s11_pdu(const srslte::gtpc_pdu& pdu);
  void handle_s11_pdu(srslte::byte_buffer_t* msg);

  virtual bool send_create_session_request(uint64_t imsi, bool cp_ciot = true);
  ...
  virtual void send_s11u_pdu(uint32_t mme_teid, srslte::byte_buffer_t* msg);
  virtual bool get_s11u_teid(uint64_t imsi, uint32_t* teid);
