...
// Service Requests
bool nas::handle_nb_control_plane_service_request(uint32_t                m_tmsi, // emm 02
                                 uint32_t                enb_ue_s1ap_id,
                                 struct sctp_sndrcvinfo* enb_sri,
                                 srslte::byte_buffer_t*  nas_rx,
                                 const nas_init_t&       args,
...
//    }
    ...
    s1ap->add_nas_ctx_to_mme_ue_s1ap_id_map(nas_ctx);
    s1ap->send_initial_context_setup_request(imsi, 5);    // emm 02
...
bool nas::pack_emm_information(srslte::byte_buffer_t* nas_buffer)
{
  ...
  strncpy(emm_info.full_net_name.name, "Toolsensing Co.,Ltd", LIBLTE_STRING_LEN);
  emm_info.full_net_name.add_ci   = LIBLTE_MME_ADD_CI_DONT_ADD;
  emm_info.short_net_name_present = true;
  strncpy(emm_info.short_net_name.name, "Toolsensing", LIBLTE_STRING_LEN);
